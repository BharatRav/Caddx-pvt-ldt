      ...
      <div className="gap-5 relative md:flex hidden">
        {NavbarConfig.map((curr, idx) => {
          ...
          {curr?.children &&<RiArrowDropDownLine color="black" />}
            ...
            {curr?.children && <div className="bg-white group-hover:grid grid-cols-2 min-w-96 grid-row-2 gap-5 hidden absolute text-black">{
              curr?.children?.map((child,cidx)=>{
                return <div key={cidx} className="whitespace-nowrap p-2">
